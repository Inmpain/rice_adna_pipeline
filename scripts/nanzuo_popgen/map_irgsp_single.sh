...
# =====================================================================
# map_irgsp_single.sh
# 阶段② 最终定量比对: 合并后的 FASTQ 用 Bowtie2 比对到 irgsp.fa, markdup 只标记
# 不删除, 产出 q20/25/30 计数 + 基因组覆盖度统计(全部只统计, 不删 reads)。
#
# 用法: bash map_irgsp_single.sh <sample> [threads]
#   输出:
#     nanzuo/02.map_irgsp/{sample}.dedup.bam(.bai)   # 重复只标记(0x400), 未删除
#     nanzuo/02.map_irgsp/stats/{sample}.tsv
# =====================================================================
...
bowtie2 -p "$THREADS" "${BT2_PARAMS[@]}" -x "$IRGSP_BT2_IDX" -U "$fq" 2> "$LOG_DIR/${sample}.aln.log" \
  | samtools view -@ "$THREADS" -bh -F "$EXCLUDE_FLAGS" - \
  | samtools sort -@ "$THREADS" -o "$mapped_bam" - 2>> "$LOG_DIR/${sample}.aln.log"
...
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
  "$sample" "$merged_reads" "$primary_mapped" "$dup_flagged" "$dup_rate" \
  "$q20" "$q25" "$q30" "$cov_bases" "$cov_pct" "$mean_depth" \
  > "$STATS_DIR/${sample}.tsv"

echo "[$sample] 完成: mapped=$primary_mapped dup=$dup_flagged q30=$q30 cov_bases=$cov_bases"
