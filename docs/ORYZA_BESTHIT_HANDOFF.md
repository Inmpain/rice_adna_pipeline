...
`--apply`还没跑——这个不影响besthit的KEEP/REJECT判定，只影响6.1节物种
contig数统计表的准确性，可以和besthit v2实测并行推进，谁先谁后不重要。
...
一个粗糙版本（末端N个位置的C→T/G→A频率 vs 距末端距离），不需要跑完整
mapDamage2：
...
### 7.6【2026-08-11新增，来自ecotype-pca-panel分支GPT策略讨论转发】按taxonomic tier分级输出KEEP集合

`codex/ecotype-pca-panel`分支跟GPT讨论PCA策略时提出的建议：当前v2的KEEP
判定是"整个Oryza属都算目标"（见5.1b节），但下游PCA用的三个panel
(`29M_3k`/`6.7M_720`/新引入的Civáň 2019统一面板，见该分支
`docs/ECOTYPE_PCA_PANEL.md`第5节)主要覆盖**sativa/rufipogon/nivara这个
目标AA基因组复合群**，不是全部18个Oryza种。如果besthit的KEEP集合把其他
15个Oryza种的reads也混进去喂给PCA，这些reads会被`smartpca -lsqproject`
强行投影到某个不相关的现代栽培/野生群附近，产生看起来"有结果"但实际上
没有生物学意义的虚假信号。

**建议**（尚未评估是否采纳，是否要做需要先确认对v2代码/输出格式的改动量）：
`<sample>.besthit_oryza.fastq.gz`按taxonomic tier拆成分级输出，而不是
现在这样只有一份"全Oryza属都算KEEP"的FASTQ：
- `oryza_genus.fastq.gz`：现在的KEEP集合原样保留，回答"是不是Oryza属"这
  一级问题（对应`docs/RESEARCH_ROADMAP.md`证据阶梯层级1）
- `target_aa_complex.fastq.gz`：从KEEP集合里进一步筛出`best_oryza`落在
  sativa/rufipogon/nivara(或AA基因组复合群其他种)的子集，这份才是真正该
  喂给PCA的输入
- `other_oryza.fastq.gz`：KEEP集合里`best_oryza`是其他15个Oryza种的部分，
  单独留存审计，不喂给PCA
- `read_taxon_assignment.tsv`：每条KEEP read的`best_oryza`具体taxon（这个
  信息`decisions.tsv`里其实已经有了，`best_oryza_taxid`/`best_oryza_name`
  两列，不一定需要新增文件，可能只是从现有`decisions.tsv`筛选/改名即可，
  不一定要在`oryza_besthit_damage_filter.py`里新增逻辑——具体要不要新增
  代码，还是靠下游脚本对现有输出做后处理，接手这条待办时先评估这一点，
  可能比想象的改动量更小）

**这条待办的性质**：不阻塞当前besthit工作（16样本v2正式批已经跑完，见0/
6.3节），是否采纳、什么时候做，由接手人根据PCA分支实际进度决定——如果
PCA分支近期就要开始写pseudo-haplotype调用脚本，需要用到"目标AA复合群
reads"这个子集，那么这条待办优先级会上升；如果PCA分支还在等Civáň数据
路径确认等前置问题（见该分支`docs/ECOTYPE_PCA_PANEL.md`5.6节），这条可以
先放着。
...
**2026-08-11：ecotype-pca-panel分支GPT策略讨论也独立提出了类似的功能位点
思路(Civáň 2019论文的qSH1/qSD1-2/SPS1三个驯化基因)，同样先建议做覆盖度QC，
两边结论一致，见该分支`docs/ECOTYPE_PCA_PANEL.md`第5.4节。**
...
4. 【诊断，优先级降低，非阻塞】7.2（`db/3k/wild/`物种身份，与main分支
   ...
   顺手`seff`）、**7.6（新增，按taxonomic tier分级输出KEEP集合，来自
   ecotype-pca-panel分支GPT讨论转发，是否采纳/优先级取决于PCA分支进度，
   见7.6节）**——都可以做，但不再挡着best-hit往前走。
