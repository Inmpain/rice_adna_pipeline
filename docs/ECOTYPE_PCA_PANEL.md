...
### 1.3 Civáň 2019 统一栽培-野生面板（PCA-C，2026-08-11新增，见第5节）

- 论文引用与数据集DOI：main分支`docs/LITERATURE.md`第2.2节（已用WebFetch/
  WebSearch核实，非转述）
- 用户已下载到服务器，**确切路径未知**——目前只看到过服务器终端里
  `(base) [yinmt@login01 paper1]$` 这个提示符和`Table_S1/S2/S3.csv`的
  `head`预览，不知道`paper1`相对哪个目录、核SNP矩阵VCF和1825个叶绿体
  基因组分别放在哪，第5节展开讨论
- 核SNP矩阵：1,056份样本(595栽培[283 indica/154 japonica/124 aus/34
  ...
  3.2节step2历史记录)
- 叶绿体基因组集：1,825个，独立于核基因组的母系谱系证据层
...
VCF对应的`inputformat`/文件后缀要求，不要想当然)。
...
用户已把数据下载到服务器，但截至本次整理，只看到终端`(base)
[yinmt@login01 paper1]$`这个提示符和三张表`head`预览，以下信息**还
不知道**，需要用户在服务器上核对贴回来：

```bash
# 1. 确认paper1目录的绝对路径、目录结构
pwd
ls -la

# 2. 核SNP矩阵是VCF格式还是别的？文件名、大小、行数
ls -la *vcf* 2>/dev/null
zcat <VCF文件名> 2>/dev/null | grep -v "^##" | head -3
zcat <VCF文件名> 2>/dev/null | grep -vc "^#"   # 应该接近2,365,188

# 3. 叶绿体基因组集：1825个序列是分开的FASTA还是合并成一个多序列FASTA？
grep -c "^>" <叶绿体fasta文件名>   # 应该接近1825

# 4. Table_S1/S2/S3.csv 完整行数(不只是head预览)
wc -l Table_S1.csv Table_S2.csv Table_S3.csv

# 5. 核对染色体命名/坐标系跟本项目其他panel是否一致(参考
#    scripts/ecotype_pca/check_ref.py 的做法，抽查REF碱基是否匹配irgsp.fa)
```

跑完贴回来，我按`file_path.md`的服务器目录总览格式把Civáň数据的确切路径
记录进`db/`目录清单(main分支`file_path.md`第三节)，并开始规划convertf
转换脚本。这是第5.1节三级框架能不能真正动手的前提，目前是本分支
最优先待办(3.2节第0条)。
