...
用BLAST验证这些落在重复区的read，是否能以相近比对分数匹配到基因组上多个不同位置
（如果是，实锤证明是重复序列导致的多重比对歧义，之前观察到的"多样本一致性"
可信度需要打折扣）：
...
# 如果还没建过blast数据库:
# makeblastdb -in /home/scratch/yinmt202607/db/asian_rice_panel_index/irgsp.fa -dbtype nucl

blastn -query test_read.fa \
    -db /home/scratch/yinmt202607/db/asian_rice_panel_index/irgsp.fa \
    -outfmt "6 qseqid sseqid pident length mismatch qstart qend sstart send evalue bitscore" \
    -word_size 11 -evalue 1e-3
```

## 建议给老师的汇报要点(如实汇报，包含过程性发现)

1. 尝试了基因±1kb扩展，未观察到侧翼调控区有效覆盖
2. 发现部分基因存在多样本reads聚集现象，但经参考序列核查，更可能是低复杂度
   重复基序导致的比对歧义，而非真实富集信号
3. 现有read长度(30-50bp)+现有深度，难以可靠区分"真实变异"与"比对/损伤artifact"
4. 建议：若要继续这个方向，可能需要更长read长度数据，或改用能识别多重比对
   歧义的专门校验流程(如BLAST交叉验证)，而非直接依赖单次BWA比对结果
