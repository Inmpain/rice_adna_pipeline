...
python3 23_validate_snp_ref_against_fasta.py \
  ...
  --fasta /home/scratch/yinmt202607/db/asian_rice_panel_index/irgsp.fa \
  --contig-format chr%02d \
  --out "$WORK/3k.ref_vs_fasta.report.tsv"
...
python3 23_validate_snp_ref_against_fasta.py \
  ...
  --fasta /home/scratch/yinmt202607/db/asian_rice_panel_index/irgsp.fa \
  --contig-format chr%02d \
  --out "$WORK/720.ref_vs_fasta.report.tsv"
```
...
- **REF/ALT 方向（关键）**：pileupCaller 用 `samtools mpileup -f irgsp.fa`，参考
  ...
  - **必须两边一起归一化到 irgsp**：
    - 不靠“猜原始 PLINK 反没反”，而是给每 panel 生成一份 **`SNP_ID + irgsp_base`**
      参考等位基因清单（直接从 irgsp.fa 按位点取），再用
      `plink --a2-allele <ref_list> 2 1 --make-bed` 强制 A2 = irgsp 碱基，PLINK 会
      自动把 genotype 按需要翻转。这样 3K / Civán / 720 三种情况统一处理，REF/ALT
      正确是构造保证。
    - `convertf` EIGENSTRAT→PACKEDPED 的映射是 `.snp` 第5列(REF)→`.bim` A2、
      第6列(ALT)→A1；而 PLINK 的 `--maf`/`--geno`/`--make-bed` 会按等位基因频率
      **重新排 A1/A2**。所以不是“转回 PLINK 锁一次就好”，而是**每一次写新 bed 后
      都要重新 `--a2-allele <ref_list> 2 1 --keep-allele-order` 锁一次**（尤其 07
      的 plink2 MAF 步骤，plink2 用 `--ref-allele` 语义）。否则 07 之后 A2 又可能
      被重排成 major allele，方向再次乱掉。
...
## 7.1 PCA 基准轴策略与验证（2026-08-18 更新）
...
本身并不“过度杀伤”。因此古代投影的 marker 必须保持 coverage 限定，不能用物理抽稀替代。
...
2. **现代诊断线（增强）**：三档现代-only PCA，其中第三档用**物理距离抽稀
   ...
   并在建轴前做**异常样本排查**（PC2/PC4 长尾、极低覆盖、疑似污染），记录剔除理由。

3. **验证线（古代 calling）**：ANGSD `-GL` 只对 16 个古代 BAM 算基因型似然，按覆盖度
   分层与 `pileupCaller --randomHaploid` 的硬分型对比，作为低覆盖调用可靠性证据。

### 7.1.4 古代覆盖位点成簇（capture 富集）

实测（2026-08-18）：720 的 MAF∩coverage 候选 872 个位点，用 `--bp-space 5000` 后
只剩 433 个（删 50%）。872 个位点落在 ~370Mb 基因组上却有一半彼此相距 <5kb，说明
古代样本的覆盖**成簇**，存在 capture 富集成分（与 `pooled_mixed_capture_plus_shotgun`
一致）。

结论：

- 古代投影**保留 r² LD（795）**，不用 bp-space——簇状位点用物理距离抽稀会过度删除。
- `--bp-space 5000` 只用于**现代 panel 诊断**（现代位点均匀，抽稀合理）。
- 修正：7.1.2 里“物理抽稀”表述仅针对现代诊断，不针对古代投影 marker。

### 7.1.3 说明

- 完整的“ANGSD GL + ngsCovar 现代建轴”对照**不可行**：服务器没有 720 现代样本的
  BAM/CRAM，只有硬基因型矩阵，无法算现代 GL。故退化为上述第 2、3 条。
